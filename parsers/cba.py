...
def _is_layout_d(pages: list) -> bool:
    ...
    for page in pages[:2]:
        ...
        desc_xs = [w["x0"] for w in header if w["text"].lower() == "description"]
        if not desc_xs:
            # No explicit "description" column on this page — can't run the day-name
            # heuristic, so we can't confirm Layout D from this page. Try the next one.
            continue
        date_x_max = min(desc_xs) - 5
