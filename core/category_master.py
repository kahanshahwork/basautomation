...
def sync_categories_safe():
    ...
    Guarded by a Postgres advisory lock so concurrent gunicorn workers don't
    double-insert the default categories.
    """
    conn = get_db()
    conn.execute("SELECT pg_advisory_lock(727352)")
    try:
        _sync_categories_impl(conn)
    finally:
        conn.execute("SELECT pg_advisory_unlock(727352)")
        conn.commit()


def _sync_categories_impl(conn):
