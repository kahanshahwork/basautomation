...
@workflow_bp.route("/statements/<int:sid>/transactions", methods=["POST"])
def add_transaction(sid):
    """Manually add a transaction row to a statement (from the Approve page's
    'Add Transaction' button). The new row is a first-class transaction: it flows
    through categorize, GST, P&L and vendor memory exactly like a parsed one.
    Body: {date, description, amount}. Returns the created row."""
    b = request.json or {}
    conn = get_db()
    stmt = conn.execute("SELECT id FROM statements WHERE id = ?", (sid,)).fetchone()
    if not stmt:
        return jsonify({"error": "Statement not found"}), 404

    try:
        amount = float(str(b.get("amount", 0)).replace("$", "").replace(",", "").strip() or 0)
    except (TypeError, ValueError):
        amount = 0.0

    # Give it a stable, obviously-manual transaction_id
    n = conn.execute("SELECT COUNT(*) AS c FROM transactions WHERE statement_id = ?", (sid,)).fetchone()["c"]
    txn_id = f"manual_{sid}_{n+1}"

    cur = conn.execute(
        """INSERT INTO transactions
           (statement_id, transaction_id, date, description, amount, balance,
            source_page, row_top, confidence, approved, category_id, gst_amount, net_amount)
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (sid, txn_id, str(b.get("date", "")).strip(), str(b.get("description", "")).strip(),
         amount, None, None, None, 1.0, 0, None, 0.0, amount),
    )
    conn.commit()
    new_id = cur.lastrowid
    log_audit("transaction", new_id, "manual_add", f"stmt={sid}")
    row = conn.execute("SELECT * FROM transactions WHERE id = ?", (new_id,)).fetchone()
    return jsonify(_row_to_txn(row))


@workflow_bp.route("/transactions/<int:tid>", methods=["DELETE"])
def delete_transaction(tid):
    """Delete a single transaction (manual or parsed) from its statement."""
    conn = get_db()
    row = conn.execute("SELECT id FROM transactions WHERE id = ?", (tid,)).fetchone()
    if not row:
        return jsonify({"error": "Not found"}), 404
    conn.execute("DELETE FROM transactions WHERE id = ?", (tid,))
    conn.commit()
    log_audit("transaction", tid, "delete")
...
def categorize(sid):
    ...
    for tid in tids:
        ...
        # NOTE: vendor memory is intentionally NOT written here. Per the project's
        # standing rule, VM is only ever written on an explicit user action via the
        # /update-vendor-memory route (the "Add to Vendor Memory" button), never
        # automatically on categorization.
    conn.commit()
...
def suggest(sid):
    """Vendor-memory suggestions for every uncategorized row in this statement.
    Suggestions are DIRECTION-VALIDATED: a suggested category's pnl_group must be
    valid for the transaction's direction (CR -> Income/Excluded; DR -> Direct Cost/
    Expense/Excluded). This guarantees a credit-only category is never suggested for a
    debit row even if stale/legacy vendor-memory data would otherwise return it."""
    conn = get_db()
    client_id = get_client_id_for_statement(conn, sid)
    rows = conn.execute(
        "SELECT * FROM transactions WHERE statement_id = ? AND category_id IS NULL", (sid,)
    ...
    CR_GROUPS = {"Income", "Excluded"}
    DR_GROUPS = {"Direct Cost", "Expense", "Excluded"}

    suggestions = {}
    if client_id:
        for r in rows:
            amount = r["amount"]
            cat_id = vendor_memory.suggest_category(client_id, r["description"], amount)
            if not cat_id:
                continue
            # Direction guard — reject a suggestion whose group doesn't fit the row's sign
            cat = category_master.get_category(cat_id)
            if not cat:
                continue
            allowed = CR_GROUPS if (amount is not None and float(amount) >= 0) else DR_GROUPS
            if cat["pnl_group"] in allowed:
...
# ── Advisors (top of the hierarchy: Advisor -> Client -> Year -> Quarter) ──

@workflow_bp.route("/advisors", methods=["GET", "POST"])
def advisors():
    conn = get_db()
    if request.method == "POST":
        ...
        cur = conn.execute(
            "INSERT INTO advisors (name, firm, email) VALUES (?,?,?)",
            (name, b.get("firm"), b.get("email")),
        )
        conn.commit()
        log_audit("advisor", cur.lastrowid, "create", detail=name)
        return jsonify({"id": cur.lastrowid, "name": name, "firm": b.get("firm"), "email": b.get("email")})
    # GET — list advisors with a client count for the sidebar
    rows = conn.execute(
        "SELECT a.*, (SELECT COUNT(*) FROM clients c WHERE c.advisor_id = a.id) AS client_count "
        "FROM advisors a ORDER BY a.name"
    ).fetchall()
    return jsonify([dict(r) for r in rows])


@workflow_bp.route("/advisors/<int:advisor_id>", methods=["PATCH", "DELETE"])
def advisor_detail(advisor_id):
    ...
    vals.append(advisor_id)
    conn.execute(f"UPDATE advisors SET {', '.join(sets)} WHERE id = ?", vals)
    conn.commit()
...
def clients():
    ...
    if request.method == "POST":
        ...
        cur = conn.execute(
            "INSERT INTO clients (name, business_type, advisor_id) VALUES (?, ?, ?)",
            (b["name"], business_type, b.get("advisor_id")),
        )
        conn.commit()
        log_audit("client", cur.lastrowid, "create", detail=f"business_type={business_type}")
        return jsonify({"id": cur.lastrowid})
    # GET — optionally filtered by advisor_id for the sidebar tree
    advisor_id = request.args.get("advisor_id", type=int)
    if advisor_id is not None:
        rows = conn.execute("SELECT * FROM clients WHERE advisor_id = ? ORDER BY name", (advisor_id,)).fetchall()
    else:
...
def quarters():
    ...
    if request.method == "POST":
        b = request.json or {}
        # Derive year from period_start if not explicitly provided (AU financial year)
        year = b.get("year")
        if not year and b.get("period_start"):
            year = _au_financial_year(b["period_start"])
        cur = conn.execute(
            "INSERT INTO quarters (client_id, year, label, period_start, period_end) VALUES (?,?,?,?,?)",
            (b["client_id"], year, b["label"], b.get("period_start"), b.get("period_end")),
        )
        conn.commit()
        return jsonify({"id": cur.lastrowid, "year": year})
...
def _au_financial_year(date_str: str) -> str:
    """Australian FY runs Jul 1 -> Jun 30. A date in Aug 2024 -> 'FY 2024-25'."""
    try:
...
@workflow_bp.route("/clients/<int:client_id>/years", methods=["GET"])
def client_years(client_id):
    """Distinct years for a client (for the sidebar tree), each with its quarters."""
    conn = get_db()
    rows = conn.execute(
        "SELECT * FROM quarters WHERE client_id = ? ORDER BY year DESC, period_start, label",
        (client_id,),
    ).fetchall()
    years: dict = {}
    for r in rows:
        yr = r["year"] or "Unassigned"
        years.setdefault(yr, []).append(dict(r))
    out = [{"year": yr, "quarters": qs, "quarter_count": len(qs)} for yr, qs in years.items()]
    # Sort: real FY years descending, "Unassigned" last
    out.sort(key=lambda x: (x["year"] == "Unassigned", x["year"]), reverse=False)
    return jsonify(out)
...
def _build_ai_categorize_prompt_text(conn, sid: int):
    """Return (system_text, user_prompt) for running AI categorization directly through
    a provider. Reuses the exact same prompt content as the /ai-categorize/prompt route
    (categories, direction rules, vendor hints), joining all batches into one prompt.
    Returns (None, None) if there are no uncategorized transactions."""
    client_id = get_client_id_for_statement(conn, sid)
    business_type_label = "Unknown"
    if client_id:
        row = conn.execute("SELECT business_type FROM clients WHERE id = ?", (client_id,)).fetchone()
    ...
    txns = conn.execute(
        """SELECT * FROM transactions WHERE statement_id = ? AND category_id IS NULL
           ORDER BY date ASC, id ASC""",
        (sid,)
    ).fetchall()
    if not txns:
    ...
    header = "ID\tDate\tDescription\tAmount\tDR/CR"
    rows_text = "\n".join(
        f'{t["transaction_id"] or t["id"]}\t{t["date"] or ""}\t{_clean_description(t["description"])}\t{abs(t["amount"]):.2f}\t{"DR" if t["amount"] < 0 else "CR"}'
        for t in txns
    )
    system = f"You are an Australian bookkeeping assistant. Business type: {business_type_label}."
    prompt = (
        f"=== CATEGORIES (use EXACT names, case-sensitive) ===\n{cat_list_str}\n\n"
        f"=== DIRECTION RULES (HARD — never break these) ===\n"
        f"DR (debit, money OUT) → ONLY: Direct Cost, Expense, or Excluded\n"
        f"  Direct Cost: {', '.join(direct_cost_cats)}\n"
        f"  Expense: {', '.join(expense_cats)}\n"
        f"  Excluded: {', '.join(excl_cats)}\n"
        f"CR (credit, money IN) → ONLY: Income or Excluded\n"
        f"  Income: {', '.join(income_cats)}\n"
        f"  Excluded: {', '.join(excl_cats)}\n"
        f"NEVER assign an Income/Direct Cost category to a DR row's opposite; keep direction correct.\n\n"
        f"=== AUSTRALIAN VENDOR HINTS ===\n{_AU_VENDOR_HINTS}\n\n"
        f"=== TRANSACTIONS ===\n{header}\n{rows_text}\n\n"
        f"=== YOUR RESPONSE ===\n"
        f"Output ONLY one line per transaction in EXACTLY this format (nothing else):\n"
        f"<ID>: <Category Name>"
    )
...
def ai_categorize_apply(sid):
    ...
    for line in response_text.splitlines():
        ...
        if client_id:
            vendor_memory.remember(client_id, txn["description"], cat["id"], amount)
...
def consolidate_quarter(qid):
    """Merge transactions from multiple statements (PDF and/or Excel/CSV sourced —
    the merge is bank-agnostic) into one consolidated statement. Name is mandatory."""
    b = request.json or {}
    stmt_ids = b.get("statement_ids", [])
    name = (b.get("name") or "").strip()
    if not stmt_ids:
        return jsonify({"error": "Select at least one statement to consolidate"}), 400
    if len(stmt_ids) < 2:
        return jsonify({"error": "Select at least two statements to consolidate"}), 400
    if not name:
        return jsonify({"error": "A name for the consolidated statement is required"}), 400
...
def update_vendor_memory_for_txn(sid):
    """Write vendor memory for one transaction (body: {transaction_id}) OR for every
    categorized transaction in the statement (body: {all: true}). Called explicitly by
    the user via the 'Add to Vendor Memory' / 'Update Categories' button — this is the
    ONLY path that writes vendor memory. Uses vendor_memory.remember() so both the exact
    pattern and the semantic-bucket pattern are learned."""
    b = request.json or {}
    conn = get_db()
    client_id = get_client_id_for_statement(conn, sid)
    if not client_id:
    ...
    if b.get("all"):
        rows = conn.execute(
            "SELECT * FROM transactions WHERE statement_id = ? AND category_id IS NOT NULL",
            (sid,),
        ).fetchall()
        learned = 0
        for row in rows:
            vendor_memory.remember(client_id, row["description"], row["category_id"], row["amount"])
            learned += 1
        log_audit("statement", sid, "update_vendor_memory", f"bulk: {learned} txns")
        return jsonify({"updated": True, "count": learned})

    txn_id = b.get("transaction_id")
    if not txn_id:
        return jsonify({"error": "transaction_id or all:true required"}), 400
    row = conn.execute(
        "SELECT * FROM transactions WHERE id = ? AND statement_id = ?", (txn_id, sid)
    ).fetchone()
    if not row or not row["category_id"]:
        return jsonify({"error": "Transaction not found or not yet categorized"}), 400

    vendor_memory.remember(client_id, row["description"], row["category_id"], row["amount"], force=True)
...
@workflow_bp.route("/clients/<int:client_id>/vendor-memory/rebuild", methods=["POST"])
def rebuild_vendor_memory(client_id):
    """One-shot maintenance: re-normalize every merchant:: pattern with the current
    merchant_key logic (fixes stale patterns like 'shein xx' -> 'shein') and collapse
    duplicates that now map to the same key, keeping the highest hit_count / majority
    category. Exact and bucket patterns are left as-is. Safe to run repeatedly."""
    from core.vendor_memory import merchant_key, _MERCHANT_PREFIX
    conn = get_db()
    rows = conn.execute(
        "SELECT * FROM vendor_memory WHERE client_id = ? AND pattern LIKE ?",
        (client_id, _MERCHANT_PREFIX + "%"),
    ).fetchall()

    # Group by corrected pattern
    corrected = {}   # new_pattern -> {category_id, hits}
    to_delete = []
    for r in rows:
        # pattern form: merchant::DIR:key
        try:
            _, rest = r["pattern"].split(_MERCHANT_PREFIX, 1)
            dir_, raw_key = rest.split(":", 1)
        except ValueError:
            continue
        new_key = merchant_key(raw_key)
        if not new_key:
            to_delete.append(r["id"])
            continue
        new_pattern = f"{_MERCHANT_PREFIX}{dir_}:{new_key}"
        entry = corrected.get(new_pattern)
        if not entry:
            corrected[new_pattern] = {"category_id": r["category_id"], "hits": r["hit_count"] or 1, "keep_id": r["id"], "ids": [r["id"]]}
        else:
            entry["ids"].append(r["id"])
            # majority category wins
            if (r["hit_count"] or 1) > entry["hits"]:
                entry["category_id"] = r["category_id"]
                entry["hits"] = r["hit_count"] or 1
                entry["keep_id"] = r["id"]

    repaired = 0
    for new_pattern, e in corrected.items():
        # keep one row (keep_id), update its pattern/category/hits, delete the rest
        total_hits = sum((conn.execute("SELECT hit_count FROM vendor_memory WHERE id = ?", (i,)).fetchone()["hit_count"] or 1) for i in e["ids"])
        conn.execute(
            "UPDATE vendor_memory SET pattern = ?, category_id = ?, hit_count = ?, updated_at = datetime('now') WHERE id = ?",
            (new_pattern, e["category_id"], total_hits, e["keep_id"]),
        )
        for i in e["ids"]:
            if i != e["keep_id"]:
                conn.execute("DELETE FROM vendor_memory WHERE id = ?", (i,))
        repaired += 1
    for i in to_delete:
        conn.execute("DELETE FROM vendor_memory WHERE id = ?", (i,))
    conn.commit()
    return jsonify({"repaired_merchant_patterns": repaired, "removed": len(to_delete)})
...
def import_bas_history(client_id):
    ...
    auto_create = mapping.get("auto_create_categories", True)  # default: discover new categories
    ...
    def _make_code(nm: str) -> str:
        base = "IMPORTED_" + _re.sub(r"[^A-Z0-9]+", "_", nm.upper()).strip("_")[:24]
        return f"{base}_{abs(hash(nm)) % 100000}"
    ...
    for row in rows:
        ...
        if not cat_id:
            if auto_create:
                # Discover a brand-new category: create it flagged is_new=1 with placeholder
                # settings. The user completes pnl_group / bas_label / GST in Category Manager,
                # which clears the "new" flag. Until then it's usable but visibly incomplete.
                try:
                    cur = conn.execute(
                        "INSERT INTO categories (code, name, pnl_group, gst_applicable, gst_rate, bas_label, is_active, is_new, sort_order) "
                        "VALUES (?,?,?,?,?,?,1,1,999)",
                        (_make_code(cat_name), cat_name, "Expense", 0, 0.10, None),
                    )
                ...
                skipped += 1
                continue
        # Derive direction from the category's P&L group (a BAS export has no signed
        # amount): Income -> CR (money in), everything else -> DR (money out).
        pg_row = conn.execute("SELECT pnl_group FROM categories WHERE id = ?", (cat_id,)).fetchone()
        pnl_group = (pg_row["pnl_group"] if pg_row else "") or ""
        cat_dir = "CR" if pnl_group == "Income" else "DR"
        vendor_memory.remember(client_id, desc, cat_id, direction=cat_dir)
        learned += 1

    conn.commit()
    log_audit("client", client_id, "import_bas_history",
              f"learned={learned}, skipped={skipped}, created={len(created_cats)}")
    return jsonify({
        "phase": 2,
        "learned": learned,
        "skipped": skipped,
        "total_rows": len(rows),
        "created_categories": sorted(set(created_cats)),
...
def update_category(cat_id):
    b = request.json or {}
    conn = get_db()
    allowed = {"name", "pnl_group", "gst_applicable", "gst_rate", "bas_label", "is_active", "is_new"}
    sets, vals = [], []
    for k, v in b.items():
        if k in allowed:
            sets.append(f"{k} = ?")
    ...
    # If any GST-affecting field changed, recompute gst_amount / net_amount for EVERY
    # transaction currently using this category — so the change reflects immediately in
    # the GST review, P&L, and exports (no need to re-categorize).
    if any(k in b for k in ("gst_applicable", "gst_rate", "bas_label", "pnl_group")):
        cat_now = conn.execute("SELECT * FROM categories WHERE id = ?", (cat_id,)).fetchone()
        if cat_now:
            cat_dict = dict(cat_now)
            affected = conn.execute(
                "SELECT id, amount FROM transactions WHERE category_id = ?", (cat_id,)
            ).fetchall()
            for t in affected:
                g = gst_engine.recalc_transaction_gst(t["amount"], cat_dict)
                conn.execute(
                    "UPDATE transactions SET gst_amount = ?, net_amount = ? WHERE id = ?",
                    (g["gst_amount"], g["net_amount"], t["id"]),
                )
            conn.commit()

    # Auto-clear the "new" flag once the category has been fully specified:
    # a valid P&L group AND a BAS label. This is the signal the user finished it.
    row = conn.execute("SELECT * FROM categories WHERE id = ?", (cat_id,)).fetchone()
    if row and row["is_new"] and row["pnl_group"] and row["bas_label"]:
        conn.execute("UPDATE categories SET is_new = 0 WHERE id = ?", (cat_id,))
        conn.commit()
        row = conn.execute("SELECT * FROM categories WHERE id = ?", (cat_id,)).fetchone()
...
# Only NEW (import-discovered) categories are checked. A pair is flagged when the
# imported category shares at least one SIGNIFICANT word with an existing category
# (ignoring stopwords and normalizing & / and, plurals, punctuation, spacing).

_DUP_STOPWORDS = {"and", "the", "of", "for", "to", "in", "on", "a", "an", "&", "expense", "expenses", "cost", "costs", "fee", "fees", "general", "other", "misc", "income"}


def _dup_words(name: str) -> set:
    import re as _re
    ...
    for w in s.split():
        ...
        if not w:
            continue
        if w.endswith("s") and len(w) > 3:   # crude singularization: expenses -> expense
            w = w[:-1]
        words.add(w)
    return words
...
def _significant_words(name: str) -> set:
    return _dup_words(name) - _DUP_STOPWORDS


@workflow_bp.route("/categories/potential-duplicates", methods=["GET"])
def potential_duplicates():
    """For every NEW (is_new=1) category, find EXISTING (non-new) categories that are
    likely the same thing. A NEW category is only ever compared against established
    categories — never against another NEW one (two freshly-imported categories
    flagging each other is noise, not a useful signal).

    Match rule (accuracy-tuned): the pair must EITHER
      (a) share ALL significant words (same word set — e.g. 'Office Expense' vs
          'Office Expenses', 'Repairs & Maintenance' vs 'Repairs and Maintenance'), OR
      (b) one significant-word set is a subset of the other (e.g. 'Fuel' vs 'MV Fuel'),
    rather than merely sharing any single word."""
    conn = get_db()
    rows = [dict(r) for r in conn.execute("SELECT * FROM categories WHERE is_active = 1").fetchall()]
    new_cats     = [c for c in rows if c.get("is_new")]
    existing_cats = [c for c in rows if not c.get("is_new")]   # ← only compare against these

    words = {c["id"]: _significant_words(c["name"]) for c in rows}
    ...
    for nc in new_cats:
        ...
        for other in existing_cats:
            ...
            if not shared:
                continue
            # Require a strong overlap, not just one incidental shared word.
            same_set = nw == ow
            subset   = nw <= ow or ow <= nw
            if same_set or subset:
                matches.append({
                    "category": {k: other[k] for k in ("id", "name", "code", "pnl_group", "bas_label", "is_new", "gst_applicable")},
                    "shared_words": sorted(shared),
                    "strength": "exact" if same_set else "subset",
                })
        if matches:
            matches.sort(key=lambda m: (m["strength"] != "exact", m["category"]["name"]))
            out.append({
                "new_category": {k: nc[k] for k in ("id", "name", "code", "pnl_group", "bas_label", "is_new", "gst_applicable")},
                "matches": matches,
            })
    return jsonify({"pairs": out, "count": len(out)})


@workflow_bp.route("/categories/merge", methods=["POST"])
def merge_categories():
    """body: {from_id, into_id}. Moves all transactions + vendor memory from `from_id`
    to `into_id`, then deletes `from_id`. `into_id` is the category you keep."""
    b = request.json or {}
    from_id = b.get("from_id")
    into_id = b.get("into_id")
    if not from_id or not into_id or from_id == into_id:
        return jsonify({"error": "Provide distinct from_id and into_id"}), 400

    conn = get_db()
    keep = conn.execute("SELECT * FROM categories WHERE id = ?", (into_id,)).fetchone()
    loser = conn.execute("SELECT * FROM categories WHERE id = ?", (from_id,)).fetchone()
    if not keep or not loser:
        return jsonify({"error": "Category not found"}), 404

    moved_txns = conn.execute(
        "UPDATE transactions SET category_id = ? WHERE category_id = ?", (into_id, from_id)
    ).rowcount

    # Move vendor memory, avoiding UNIQUE(client_id, pattern) collisions
    vms = conn.execute("SELECT * FROM vendor_memory WHERE category_id = ?", (from_id,)).fetchall()
    moved_vm = 0
    for vm in vms:
        exists = conn.execute(
            "SELECT id FROM vendor_memory WHERE client_id IS ? AND pattern = ?",
            (vm["client_id"], vm["pattern"]),
        ).fetchone()
        # A row for the SAME pattern may already point elsewhere; if it's the loser's, repoint it,
        # otherwise (a keep-side row already exists) just drop the loser's duplicate row.
        dup_keep = conn.execute(
            "SELECT id FROM vendor_memory WHERE client_id IS ? AND pattern = ? AND category_id = ?",
            (vm["client_id"], vm["pattern"], into_id),
        ).fetchone()
        if dup_keep:
            conn.execute("DELETE FROM vendor_memory WHERE id = ?", (vm["id"],))
        else:
            conn.execute("UPDATE vendor_memory SET category_id = ? WHERE id = ?", (into_id, vm["id"]))
            moved_vm += 1

    conn.execute("DELETE FROM categories WHERE id = ?", (from_id,))
    conn.commit()
    log_audit("category", into_id, "merge", f"merged {loser['name']} -> {keep['name']}, txns={moved_txns}, vm={moved_vm}")
    return jsonify({
        "merged": True, "kept": keep["name"], "removed": loser["name"],
        "moved_transactions": moved_txns, "moved_vendor_memory": moved_vm,
    })


@workflow_bp.route("/categories/<int:cat_id>/dismiss-new", methods=["POST"])
def dismiss_new(cat_id):
    """'Keep both' — clear the is_new flag so the category stops being flagged as a
    potential duplicate, without requiring pnl_group/bas_label completion."""
    conn = get_db()
    conn.execute("UPDATE categories SET is_new = 0 WHERE id = ?", (cat_id,))
    conn.commit()
    return jsonify({"dismissed": cat_id})
...
def approve_and_learn(sid):
    ...
    for t in rows:
        vendor_memory.remember(client_id, t["description"], t["category_id"], t["amount"])
