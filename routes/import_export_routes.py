...
def export_pnl(sid):
    ...
    try:
        ...
        # Header
        ws.append(["Category", "P&L Group", "GST Unadjusted (Gross)", "GST Adjusted (Net)", "GST", "Txn Count"])
        ...
        for row in sorted_rows:
            ...
            gst_amt   = round(abs(gross_amt) - abs(net_amt), 2)   # GST = gross − net (the 1/11th)
            total_gst += gst_amt

            ws.append(["  " + (row.get("category") or ""), grp, gross_amt, net_amt, gst_amt, row.get("count", 0)])
            r = ws.max_row
            for c in [3, 4, 5]:
                ws.cell(r, c).number_format = "#,##0.00"
                ws.cell(r, c).alignment     = right
            # GST column tinted purple-ish to match the P&L page
            ws.cell(r, 5).font = Font(color="7C3AED")
        ...
        # Totals
        def tot_row(label, gross_val, net_val, gst_val, fill):
            ws.append([label, "", gross_val, net_val, gst_val, ""])
            r = ws.max_row
        ...
        tot_row("NET PROFIT / LOSS", gross_net, adj_net, round(total_gst, 2), net_fill)
